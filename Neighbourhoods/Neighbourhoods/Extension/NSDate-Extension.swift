...
extension NSDate {
    ...
    class func createDateString(createAtStr : String) -> String {
       ...
       fmt.locale = NSLocale(localeIdentifier: "en") as Locale!
        ...
        // 2.将字符串时间,转成NSDate类型
        let  time = Double(createAtStr)
        let createDate  = Date.init(timeIntervalSince1970: time!)
...
//        }
        ...
        let cmps = calendar.dateComponents(component, from: createDate, to: nowDate as Date)
        if cmps.year! < 1 {
